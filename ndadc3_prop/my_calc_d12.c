                  ...
                  double *d12_matrix, double *cap_elements)
{
  ...
  int num_rows, x;                   /* for debugging purposes */
  int index_i, index_j;
  int dim_2h1p;
  ...
  FOR_ALL_2H1P_AKL({
	  ...
	  FOR_VIR_IN_SYM_WE(p,p_sym,epsi_p){
		...
		(2.0 *V1212(q,l,p,a) - V1212(q,l,a,p))
		 / (epsi_q + epsi_l - epsi_p - epsi_a);
	      ...
	      wij_1 -= SQRT_1_2 * sum_1 * CAP_PTR(p,q);  
	    }
	  }
	}
      }   	
	  ...
	  FOR_VIR_IN_SYM_WE(p,p_sym,epsi_p){
		...
		(2.0 *V1212(q,l,p,a) - V1212(q,l,a,p))
		 / (epsi_q + epsi_l - epsi_p - epsi_a);
	      ...
	      wij_2 += SQRT_3_2 * sum_1 * CAP_PTR(p,q);  
	    }
	  }
	}
      }   
	...
	sum_1 = -V1212(k,k,a,p) / (epsi_k + epsi_k - epsi_a - epsi_p);
...
/*   /\* debug *\/ */
/*   if(inp->debug>2){ */
/*     num_rows=symtab->nOcc[inp->sym]; */
/*     dim_2h1p=calc_dim_2h1p(inp,symtab); */
/*     for(x=0; x< dim_2h1p*num_rows ; x++){ */
/*       calc_indexpair_cf(num_rows,x,&index_i,&index_j); */
/*       printf("global index: %i, index_pair (%i,%i), element: %16.14f\n", x, index_i, index_j, d12_matrix[x]); */
/*     } */
/*   } */
